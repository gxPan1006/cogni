...
**追加(同日,inline 实现,非扇出)**:上面三个候选全部做完(`daf77ef` + `9f81c84`)。
- `--append-system-prompt`:preamble 改为每轮随 dispatch 帧的 `appendSystemPrompt` 下发(survives `--resume`,聊天气泡只显示原始用户文本)。
- project default host:project-scoped 线程优先 `project.defaultHostId`(在 last-session host 之后,保 `--resume` 连续性)。
- per-task host override:NewTask 加 host 选择器(>1 host 才显示),`task.hostId` 贯穿 create→route(校验归属)→db→orchestrator dispatch,缺省回退 `project.defaultHostId`。
- 三件因共享 contract + cloud 文件,无法干净划主权 → 按 playbook「不满足就自己干」inline 实现。全量 491 tests 绿。
